...
def train():
    ...
    for agent in agents:
        ...
        print(
            f"[INFO] new agent train, itteration {current_itteration} of {max_itterations}, current best {best_agent_lines_cleared}, this took {end_state.rowsRemoved} "
        )
        if end_state.rowsRemoved > best_agent_lines_cleared:
            print(
                f"[UPDATE] Ny beste agent funnet med {end_state.rowsRemoved} rader fjernet."
            )
            improvement_factor = (end_state.rowsRemoved - best_agent_lines_cleared) / (
                best_agent_lines_cleared + epsilon
            )
            hyperparameters_seed = [
                seed + learning_rate * improvement_factor * (agent_param - seed)
                for seed, agent_param in zip(
                    hyperparameters_seed, agent.hyperparameters
                )
            ]
            best_agent = agent
            ...
            relative_improvement = (end_state.rowsRemoved - min_lines_cleared) / (
                best_agent_lines_cleared - min_lines_cleared + epsilon
            )
            hyperparameters_seed = [
                seed
                + learning_rate * relative_improvement * epsilon * (agent_param - seed)
                for seed, agent_param in zip(
                    hyperparameters_seed, agent.hyperparameters
                )
            ]
...
def train_random():
    ...
    print(
        f"Det er {len(agents)} agenter som nå skaø prøve seg med tilfeldige hyperparametere!"
    )
...
def _create_heuristic_agents(num_agents: int):
    agents = [
        HeuristicAgentHenrik(create_random_hyperparameters()) for _ in range(num_agents)
    ]
    return agents
...
def create_random_hyperparameters():
    return [
        random.uniform(-1, 0),  # aggregate_heights_weight
...
def train_beta():
    ...
    for agent in agents:
        ...
        print(
            f"[INFO] Ny agent trener, iterasjon {current_iteration} av {max_iterations}, nåværende beste {best_agent_lines_cleared}, dette tok {lines_cleared} rader"
        )
